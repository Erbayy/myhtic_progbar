...
function ProgressWithStartEvent(action, start, finish)
    ...
    if not IsEntityDead(GetPlayerPed(-1)) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("mythic_base:client:DoHudText", { text = "Already Doing An Action", type = "error", layout = "topRight", timeout = 1500 })
        end
    else
        TriggerEvent("mythic_base:client:DoHudText", { text = "Cannot Perform An Action While Dead", type = "error", layout = "topRight", timeout = 1500 })
    end
end
